...
from model.model import Model_encoder
...
parser.add_argument('--epoch',type = int, default= 1, help = 'epoch for trianing')
...
if __name__ == '__main__':
    ...
    model = Model_encoder(input_size=94,hidden_size=10,input_lang_word2index= input_lang_word2index, output_lang_word2index=output_lang_word2index)
    ...
    for epoch in range(1, args.epoch + 1):
        ...
        for name in dials:
            ...
            input_tensor = [];target_tensor = [];bs_tensor = [];db_tensor = []
            val_file = train_dials[name]
            # first to concatanate the dataset.
            input_tensor, target_tensor, bs_tensor, db_tensor = util.loadDialogue_encoder(model,input_lang_word2index, output_lang_word2index, val_file, input_tensor, target_tensor, bs_tensor, db_tensor)
            if count >100 and len(bs_tensor)>1:
                # inverse training process
                length = len(bs_tensor)
                rand = np.random.randint(low=1,high=length)
                val = bs_tensor[:rand]
                ...
                loss = model(bs_tensor,20)
                loss.backward()
                optimizer.step()
                optimizer.zero_grad()
            loss_record.append(float(loss))
        axis = [i for i in range(len(loss_record))]
        plt.plot(axis,loss_record)
        plt.xlabel('number of dialogue turns')
        plt.ylabel('RMSE loss between 20 and 0')
        plt.show()
